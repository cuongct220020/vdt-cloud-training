...
Formats like MIDI files need to store arbitrarily large integers, but most of the time those integers are actually quite small — so spending a fixed 4 bytes on every single one wastes space. Variable Length Quantity (VLQ) encoding solves that by using as few bytes as the value actually needs, at the cost of a slightly trickier encoding scheme.
